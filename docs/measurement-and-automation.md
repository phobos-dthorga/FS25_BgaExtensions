...
| --- | --- | --- | --- |
| Static validation | GitHub Actions and local tools | Implemented | XML is well-formed, core fillTypes are known, optional fillTypes are not in core XML, storage-only production warnings are prevented, l10n references resolve, recipe count stays below the hard target. |
