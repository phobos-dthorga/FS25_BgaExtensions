...
| Storage design | Every stored fillType in a production point must be used by a production input or output. Same-fillType pass-through is allowed only for the documented Process Supply Hub and Process Pallet Dock dispatcher recipes. | Any storage-only fillType warning in the game log, or any accidental same-fillType production loop outside the supply dispatchers. | Remove the storage entry, implement a real production route/storage module, or stop and redesign the dispatcher pattern. |
