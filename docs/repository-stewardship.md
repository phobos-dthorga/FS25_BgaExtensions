...
- Dual licensing added to match nearby Project Zomboid mod repositories.
...
- Visual asset policy and the custom fillType HUD icons are documented.
...
- Raw pink chroma-key AI icon sources are kept off `main`; the v0.2.14.0 single-mash source was preserved on branch `asset-backup/ai-chroma-source-v0.2.14.0`, and the v0.2.15.0 four-mash sources were preserved on branch `asset-backup/ai-chroma-source-v0.2.15.0`.
- Dry fuel yard storage is offered in medium and large PlanET-style variants; the old small variant was removed during the breaking `v0.2.15.0` GBW rebrand because its rotating blade does not fit passive storage.
...
- GBW placeables are grouped into custom construction tabs inside vanilla FS25 categories.
...
This repository follows the established pattern from nearby Project Zomboid mod repositories:
