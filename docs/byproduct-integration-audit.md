...
| `STRAW_PELLETS` | Dry Fuel Processor straw pelletizing | Straw Harvest HALLSYS Pellet Heat Plant, pellet-compatible systems, and assisted GBW fermentation | Strong cross-mod by-product/fuel route. Keep heat logistics meaningful and BGA use additive-gated; balance BGA output against raw straw equivalent. |
| `HAY_PELLETS` | Dry Fuel Processor hay pelletizing | Straw Harvest pellet-compatible systems and assisted GBW fermentation | Useful dual-purpose route; better BGA substrate than straw pellets and raw hay pretreatment, but still below prepared silage throughput. |
...
- large biomass intake raw routes -> normal `SILAGE_IN` handoff plus small recovered `ORGANICWASTE` side-streams
...
- Avoid defining `ORGANICWASTE` in GBW. It would fragment compatibility with mods that already define it. Producing it is acceptable only inside a provider-specific add-on that depends on that fillType provider.
